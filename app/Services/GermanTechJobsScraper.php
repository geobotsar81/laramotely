...
class GermanTechJobsScraper extends Scraper
{
    /**
    ...
    {
        $url = "https://germantechjobs.de/en/jobs/Laravel/all";
        $client = new Client(HttpClient::create(["timeout" => 600]));
        $crawler = $client->request("GET", $url);
        $nodes = $crawler->filter(".joblist-container li");
        ...
        foreach ($nodes as $node) {
            ...
            if (!empty($node->filter(".card-body a")->count() > 0)) {
                $url =
            ...
            if (!empty($node->filter("h2")->count() > 0)) {
                $title = $node
                    ->filter("h2")
                    ->first()
                    ->text();
            ...
            if (!empty($node->filter(".cut-job-address .mr-3")->count() > 0)) {
                $company = $node
                    ->filter(".cut-job-address .mr-3")
                    ->first()
                    ->text();
            }
